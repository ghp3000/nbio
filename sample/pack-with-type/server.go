package main

import (
	"bytes"
	"encoding/binary"
	"fmt"
	"github.com/ghp3000/nbio"
	nbtls "github.com/ghp3000/nbio/extension/tls"
	"github.com/ghp3000/nbio/logging"
	"github.com/ghp3000/nbio/mempool"
	"github.com/lesismal/llib/std/crypto/tls"
)

type Session struct {
	bytes.Buffer
}

func (s *Session) headerLen() int {
	return 5
}
func (s *Session) bodyLen() (length uint32) {
	if s.Len() < s.headerLen() {
		return 0
	}
	header := s.Bytes()[1:s.headerLen()]
	binary.Read(bytes.NewBuffer(header), binary.LittleEndian, &length)
	return
}
func main() {
	cert, err := tls.X509KeyPair(rsaCertPEM, rsaKeyPEM)
	if err != nil {
		fmt.Printf("tls.X509KeyPair failed: %v", err)
	}
	tlsConfig := &tls.Config{
		Certificates:       []tls.Certificate{cert},
		InsecureSkipVerify: true,
	}
	logging.SetLevel(0)
	g := nbio.NewEngine(nbio.Config{
		Network: "tcp",
		Addrs:   []string{"localhost:8888"},
	})

	isClient := false
	g.OnOpen(nbtls.WrapOpen(tlsConfig, isClient, func(c *nbio.Conn, tlsConn *tls.Conn) {
		session := &Session{}
		c.SetExtra(session)
		fmt.Println("OnOpen:", c.RemoteAddr().String())
	}))
	g.OnClose(nbtls.WrapClose(func(c *nbio.Conn, tlsConn *tls.Conn, err error) {
		fmt.Println("OnClose:", c.RemoteAddr().String())
	}))

	g.OnData(nbtls.WrapData(func(c *nbio.Conn, tlsConn *tls.Conn, data []byte) {
		iExtra := c.Extra()
		if iExtra == nil {
			c.Close()
			return
		}
		extra, ok := iExtra.(*Session)
		if !ok {
			c.Close()
			return
		}
		extra.Write(data)
		tlsConn.Write(data)
		if extra.Len() >= extra.headerLen() {
			bodyLen := extra.bodyLen()
			total := int(bodyLen) + extra.headerLen()
			if extra.Len() >= total {
				buffer := mempool.Malloc(total)
				if n, err := extra.Read(buffer); err == nil {
					fmt.Println(n)
					parserMsg(buffer)
				} else {
					fmt.Println(n, err)
				}
			}
		}
	}))

	err = g.Start()
	if err != nil {
		fmt.Printf("nbio.Start failed: %v\n", err)
		return
	}
	defer g.Stop()

	g.Wait()
}
func parserMsg(payload []byte) {
	r := bytes.NewBuffer(payload)
	var length uint32 //载荷/有效数据长度
	var typ uint8     //消息类型
	binary.Read(r, binary.LittleEndian, &typ)
	binary.Read(r, binary.LittleEndian, &length)
	headerLen := 1 + 4 // typ 长度1,length 长度4
	fmt.Println(typ, length, string(payload[headerLen-1:]))
	mempool.Free(payload) //使用结束,释放内存池资源
}

var rsaCertPEM = []byte(`-----BEGIN CERTIFICATE-----

DNn00G8C6ttLoGU2snyk
-----END CERTIFICATE-----
`)

var rsaKeyPEM = []byte(`-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`)
